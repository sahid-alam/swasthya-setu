...
Telegram (`telegram_mock.py` + `telegram_real.py`, `TELEGRAM_MOCK_MODE`) leads `CHANNEL_ORDER` because it is free and works on any cheap Android with data, with SMS last because it always works and always costs money. It is the one channel a patient must opt into: a bot cannot address anyone by phone number, so `services/telegram_link.py` waits for the patient to share their contact with @Swasthya_Setu_bot and stores the chat id Telegram hands over alongside the number *it* verified. Unlinked patients have no Telegram address and are skipped without an attempt. Updates are consumed by `getUpdates` polling rather than a webhook, so a laptop behind NAT needs no public URL and Telegram does not break when the demo tunnel restarts. The bot is a full booking channel, not just a delivery one: language first, then Telegram's own contact-share button, then department → time → confirm through the same `services/booking.py`. A number we do not know is **registered on the spot** rather than turned away — Telegram vouches for the number it hands over, which is a stronger claim than anything typed into a form, and `patients.registered_via` records which door they came in through. `CANCEL` is honoured because every confirmation promises it. SMS live mode is an Android running the Traccar SMS Gateway app on the LAN (`SMS_GATEWAY_URL`) — a hospital in Himachal has a phone and a SIM long before it has an MSG91 account.
...
`adapters/vapi.py` is the voice-agent boundary: Vapi runs the speech in the browser and calls our tools from its own servers, so the only vendor shape reaching us is the tool-call envelope (which Vapi has shipped both flat and nested under `function`, with arguments as an object or a JSON string — all three are accepted here and the flow sees one shape). `POST /channels/vapi/tools` is authenticated by a shared secret header rather than our JWT, because Vapi cannot hold a JWT, and it returns 503 when `VAPI_TOOL_SECRET` is unset — an unconfigured endpoint on a public URL must be closed, not open. The tools call the same `services/booking.py` as every other channel. There is deliberately no `vapi_mock.py`: the fallback for a voice agent is `simulators/vapi_call.py` posting that envelope with no internet, plus the IVR line, which books the same appointment from a feature phone.
...
| D29 | `SMS_PROVIDER` picks the live SMS implementation; mock wins if either it or `SMS_MOCK_MODE` says so | Going live on the one channel that costs money and rings a stranger's phone should take two deliberate acts, not one typo. 2Factor's voice route is live and its SMS route is off because SMS reports DELIVERED and is still carrier-filtered by India's DLT rules — a receipt from the sender is not evidence of arrival | The sender ID clears DLT |
| D30 | An adapter may decline a message kind (`MessagingAdapter.handles`), and the fan-out skips rather than failing it | 2Factor delivers digits down a phone line, so a booking confirmation is not something it *failed* to send — a FAILED row would read as "we tried and could not reach them" | — |
| D28 | Vapi reaches the local backend through a `cloudflared` quick tunnel (`make tunnel`), not a deployment | Deploying means provisioning Postgres, Redis and secrets somewhere, and the demo stops being laptop-local — which contradicts every other part of this build. The tunnel is one command, no account, and only up while demoing. It publishes the whole backend including staff login, so it is demo-time only | Judging requires a always-on public URL |
